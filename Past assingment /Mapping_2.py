...
# e.g. if mapping of full network is desired, comment sections for aircraft 1, 2, 3, 4, 5
...
# Extracting data from optimisation results (Excel file 'Model_2_results.xls')
xls = pd.ExcelFile('Model_2_results.xls')
...
ac_type_4 = pd.read_excel(xls, 'AC 4') # frequency of aircraft type 3
ac_type_5 = pd.read_excel(xls, 'AC 5') # frequency of aircraft type 3
...
AC_4_network = np.array(ac_type_4)[::,1::]
AC_5_network = np.array(ac_type_5)[::,1::]
...
#     for j in range(len(AC_1_network[i])):
...
#     for j in range(len(AC_2_network[i])):
...
#     for j in range(len(AC_3_network[i])):
...
#######################################################################################################################
##############################################   RUN FOR AIRCRAFT 4 NETWORK   #########################################
#######################################################################################################################

# # Creating dictionary associating each airport (origin) to a list of ALL its feasible destinations
# dicti_AC_4 = {}
# for i in range(len(AC_4_network)):
#     for j in range(len(AC_4_network[i])):
#         AC4_routes = AC_4_network[i][j]
#         if AC4_routes != 0:
#             if Airports[i] not in dicti_AC_4:
#                 dicti_AC_4[Airports[i]] = []
#             dicti_AC_4[Airports[i]].append(Airports[j])
#
# # Plotting on map
# for airport in Airports:
#     if airport in dicti_AC_4:
#         for i in dicti_AC_4[airport]:
#             lista = [(dictionary[airport]['Latitude_(deg)'],dictionary[airport]['Longitude_(deg)']),(dictionary[i]['Latitude_(deg)'],dictionary[i]['Longitude_(deg)'])]
#             folium.PolyLine(lista, color="red", weight=2.5, opacity=1).add_to(m)
# m.save('indexy.html')

#######################################################################################################################
##############################################   RUN FOR AIRCRAFT 5 NETWORK   #########################################
#######################################################################################################################

# # Creating dictionary associating each airport (origin) to a list of ALL its feasible destinations
# dicti_AC_5 = {}
# for i in range(len(AC_5_network)):
#     for j in range(len(AC_5_network[i])):
#         AC5_routes = AC_5_network[i][j]
#         if AC5_routes != 0:
#             if Airports[i] not in dicti_AC_5:
#                 dicti_AC_5[Airports[i]] = []
#             dicti_AC_5[Airports[i]].append(Airports[j])
#
# # Plotting on map
# for airport in Airports:
#     if airport in dicti_AC_5:
#         for i in dicti_AC_5[airport]:
#             lista = [(dictionary[airport]['Latitude_(deg)'],dictionary[airport]['Longitude_(deg)']),(dictionary[i]['Latitude_(deg)'],dictionary[i]['Longitude_(deg)'])]
#             folium.PolyLine(lista, color="red", weight=2.5, opacity=1).add_to(m)
# m.save('indexy.html')